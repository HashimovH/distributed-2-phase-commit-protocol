...
class Network:
    ...
    def handleCreation(self, id_, is_coordinator, lastValue):
        ...
        node_obj = Process(node_id, is_coordinator, self.current_port, lastValue)
        create(node_obj.id_, node_obj.is_coordinator, node_obj.port, node_obj.value)
    ...
    def process_command(self, command):
        ...
        #this is problematic, the coordinator isn't doing anything, he is treated just like other nodes
            ...
            if(N >= len(self.consistency_history)):
                ...
                for node in self.node_list:
                    message += send_message(f"value {newValue}", node.port) + "\n"
            else:
                message = "Aborted, not all nodes are active"
            ...
            check = True
            for node in self.node_list:
                if(send_message("id", node.port) == nd[1]):
                    message = "Aborted, such a Process already exists"
                    check = False
                    break
            if(check):
                self.handleCreation(nd, False, self.current_port, 2)
                message = f"{nd} added"
            ...
            if(node != None):
                t = threading.Thread(target= self.timeFailure, args = (node, secs,))
                t.start()
            ...
            node = self.findProcess(process_)
            if(node != None):
                t = threading.Thread(target= self.arbFailure, args = (node, secs,))
                t.start()
